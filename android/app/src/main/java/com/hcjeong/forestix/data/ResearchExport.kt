...
object ResearchExport {
    ...
    data class Reading(
        ...
        val treeNumber: Int?,
        val value: Double,
        /// MAY BE A TIME THE CRUISER SET BY HAND ([QuickMeasureEntry.timeSource]
        /// == "typed"), and the join below deliberately uses it anyway: a
        /// corrected time is the cruiser's best account of when the tree was
        /// measured, which is precisely what "nearest row" and "is there a
        /// later reading" are trying to ask.
        ///
        ...
        /// rewrites a raw-capture manifest. So after a hand-set time the two no
        /// longer agree, which is why the reading's own `time_source` travels in
        /// the quick-measure CSV: an analyst who needs to know whether a Δt is
        /// real can see it there.
        val createdAt: Long,
        val truth: Double?,
