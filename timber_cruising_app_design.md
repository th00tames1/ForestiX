...
4. DBH reads the depth pixels on the frame's **centre row** and nothing else (§7.1 Step 3), and the CROSSHAIR RING marks that row — it is drawn at the centre of the viewport, which is where the centre row is. The horizontal line through the ring is no longer a marker for that row: it is an ARTIFICIAL HORIZON, the ring's width, riding the camera's pitch at 4 pt per degree to a 90 pt stop and turning green inside 1.5° of level. It used to be full-width and fixed, on the reasoning that it marked the measured row — true, and useless to look at, because a line that is in the same place whatever the cruiser does can never tell them they are aiming uphill, and a stem read off a tilted phone is read across a slanted chord.
