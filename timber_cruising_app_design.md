...
4. DBH reads the depth pixels on the frame's **centre row** and nothing else (§7.1 Step 3), and TWO MARKS sit on that row, doing two different jobs. The **fixed guide line** is full-width, dim, at `y = screen_height / 2`, and NEVER MOVES: it marks the row the number comes from, and it is the one the cruiser aligns with the DBH point on the trunk. Overlaid on it is the **artificial horizon** — the crosshair ring's width, brighter, riding the camera's pitch at 4 pt per degree to a 90 pt stop and turning green inside 1.5° of level. It marks nothing on the trunk; it says how the phone is being held, because a stem read off a tilted phone is read across a slanted chord. The row marker is kept in the Accept-time photo; the horizon is not.
